...
{
	/// <summary>
	/// The ZeeGraphWeb class provides a web control interface to the
	/// <see cref="ZeeGraph"/> class library.  This allows ZedGraph to be used
	/// from a web page with ASP.net.  All graph
	/// attributes are accessible via the <see cref="GraphPane"/>
	/// property.
	/// </summary>
	...
	ToolboxData( "<{0}:ZeeGraphWeb runat=server></{0}:ZeeGraphWeb>" )
	]
	public class ZeeGraphWeb : Control, INamingContainer, IDisposable
	{
		/// <summary>
	...
	#region Constructors
		...
		public ZeeGraphWeb()
		{
			...
			vsassist.Register( 'x', typeof( ZeeGraphWebXAxis ) );
			vsassist.Register( 'y', typeof( ZeeGraphWebYAxis ) );
			vsassist.Register( 'z', typeof( ZeeGraphWebY2Axis ) );
			vsassist.Register( 'l', typeof( ZeeGraphWebLegend ) );
			vsassist.Register( 'b', typeof( ZeeGraphWebBorder ) );
			vsassist.Register( 'B', typeof( ZeeGraphWebBorder ) );
			vsassist.Register( 'f', typeof( ZeeGraphWebFill ) );
			vsassist.Register( 'F', typeof( ZeeGraphWebFill ) );
			vsassist.Register( 'E', typeof( ZeeGraphWebFill2 ) );
			vsassist.Register( 'C', typeof( ZeeGraphWebBorder2 ) );
			vsassist.Register( 's', typeof( ZeeGraphWebFontSpec ) );
			vsassist.Register( 'c', typeof( ZeeGraphWebCurveCollection ) );
			vsassist.Register( 'g', typeof( ZeeGraphWebGraphObjCollection ) );
			//vsassist.Register('r',typeof(ZeeGraphWebRect));
			//vsassist.Register('R',typeof(ZeeGraphWebRect));
			vsassist.Register( 'm', typeof( ZeeGraphWebRect ) );

			this.ChartBorder.Color = Chart.Default.BorderColor;
			this.ChartBorder.Width = Chart.Default.BorderPenWidth;
			this.ChartBorder.IsVisible = true;
			this.ChartFill.Brush = Chart.Default.FillBrush;
			this.ChartFill.Color = Chart.Default.FillColor;
			this.ChartFill.Type = Chart.Default.FillType;

			this.PaneBorder.Color = PaneBase.Default.BorderColor;
			this.PaneBorder.Width = PaneBase.Default.BorderPenWidth;
			this.PaneBorder.IsVisible = true;
			this.PaneFill.Color = PaneBase.Default.FillColor;

			this.FontSpec.IsBold = PaneBase.Default.FontBold;
			this.FontSpec.FontColor = PaneBase.Default.FontColor;
			this.FontSpec.Family = PaneBase.Default.FontFamily;
			this.FontSpec.IsItalic = PaneBase.Default.FontItalic;
			this.FontSpec.Size = PaneBase.Default.FontSize;
			this.FontSpec.IsUnderline = PaneBase.Default.FontUnderline;
			this.FontSpec.Fill.Type = FillType.None;	// no fill
			this.FontSpec.Border.IsVisible = false;

			this.XAxis.IsVisible = ZeeGraph.XAxis.Default.IsVisible;
			this.XAxis.IsZeroLine = ZeeGraph.XAxis.Default.IsZeroLine;

			this.YAxis.IsVisible = ZeeGraph.YAxis.Default.IsVisible;
			this.YAxis.IsZeroLine = ZeeGraph.YAxis.Default.IsZeroLine;

			this.Y2Axis.IsVisible = ZeeGraph.Y2Axis.Default.IsVisible;
			this.Y2Axis.IsZeroLine = ZeeGraph.Y2Axis.Default.IsZeroLine;

			ZeeGraphWebRect margins = this.Margins;
	...
	#region Data Properties
		...
		public ZeeGraphWebCurveCollection CurveList
		{
			get { return (ZeeGraphWebCurveCollection)vsassist.GetValue( 'c', this.IsTrackingViewState ); }
		}
		...
		public ZeeGraphWebGraphObjCollection GraphObjList
		{
			get { return (ZeeGraphWebGraphObjCollection)vsassist.GetValue( 'g', this.IsTrackingViewState ); }
		}
	...
	#region MasterPane Properties
		...
		*/
		...
		public ZeeGraphWebFill2 MasterPaneFill
		{
			get { return (ZeeGraphWebFill2)vsassist.GetValue( 'E', this.IsTrackingViewState ); }
		}
		...
		public ZeeGraphWebBorder2 MasterPaneBorder
		{
			get { return (ZeeGraphWebBorder2)vsassist.GetValue( 'C', this.IsTrackingViewState ); }
		}
	...
	#region GraphPane properties
		...
		public ZeeGraphWebRect Margins
		{
			get { return (ZeeGraphWebRect)vsassist.GetValue( 'm', this.IsTrackingViewState ); }
		}

		/// <summary>
		/// Proxy property that gets or sets the Title of the <see cref="GraphPane"/>.
		/// </summary>
		...
		/// determines if the <see cref="PaneBase.Title"/> is visible.
		/// </summary>
		...
		*/
		...
		{
			get { return (ZeeGraphWebFontSpec)vsassist.GetValue( 's', this.IsTrackingViewState ); }
		}
		/// <summary>
		...
		public ZeeGraphWebBorder PaneBorder
		{
			get { return (ZeeGraphWebBorder)vsassist.GetValue( 'B', this.IsTrackingViewState ); }
		}
		...
		public ZeeGraphWebFill PaneFill
		{
			get { return (ZeeGraphWebFill)vsassist.GetValue( 'F', this.IsTrackingViewState ); }
		}
	...
	#region Chart Properties
		...
		public ZeeGraphWebBorder ChartBorder
		{
			get { return (ZeeGraphWebBorder)vsassist.GetValue( 'b', this.IsTrackingViewState ); }
		}
		...
		public ZeeGraphWebFill ChartFill
		{
			get { return (ZeeGraphWebFill)vsassist.GetValue( 'f', this.IsTrackingViewState ); }
		}
		...
		public ZeeGraphWebXAxis XAxis
		{
			get { return (ZeeGraphWebXAxis)vsassist.GetValue( 'x', this.IsTrackingViewState ); }
		}
		...
		public ZeeGraphWebYAxis YAxis
		{
			get { return (ZeeGraphWebYAxis)vsassist.GetValue( 'y', this.IsTrackingViewState ); }
		}
		...
		public ZeeGraphWebY2Axis Y2Axis
		{
			get { return (ZeeGraphWebY2Axis)vsassist.GetValue( 'z', this.IsTrackingViewState ); }
		}
		...
		public ZeeGraphWebLegend Legend
		{
			get { return (ZeeGraphWebLegend)vsassist.GetValue( 'l', this.IsTrackingViewState ); }
		}
	...
	#region Behavior Properties
		...
		/// <see cref="GraphPane.AxisChange()"/> method to be called when
		...
		/// <summary>
		/// Proxy property that gets or sets the value of <see cref="ZeeGraphWeb.IsImageMap" />.
		/// </summary>
		[
	...
	#region Event Handlers
		...
		public event ZeeGraphWebControlEventHandler RenderGraph
		{
		...
		{
			ZeeGraphWebControlEventHandler handler;
			handler = (ZeeGraphWebControlEventHandler)Events[_eventRender];
	...
	#region Map Embedded Content
		...
		/// This requires applying each <see cref="ZeeGraphWebCurveItem"/> to the <see cref="GraphPane"/>
	...
	#region Render Methods
		...
		/// <summary>
		/// Method to create a <see cref="GraphPane"/> class for the control.
		/// </summary>
...
//			output.AddAttribute( HtmlTextWriterAttribute.Coords, coords );
	...
	#region IServiceProvider Members
		...
		{
			// TODO:  Add ZeeGraphWeb.GetService implementation
			return null;
		}
	...
	/// </summary>
	/// <param name="webObject">A reference to this <see cref="ZeeGraphWeb" /> object</param>
	/// <param name="g">A <see cref="Graphics"/> object for which the drawing will be done.</param>
	/// <param name="pane">A reference to the <see cref="GraphPane"/>
	/// class to be rendered.</param>
	public delegate void ZeeGraphWebControlEventHandler( ZeeGraphWeb webObject,
